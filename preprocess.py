...
def get_input_data()->pd.DataFrame:
    ...
    for label in Config.TYPE_COLS:
        df = df.loc[(df[label] != '') & (~df[label].isna()),]
    return df
...
def create_combined_labels(df: pd.DataFrame) -> pd.DataFrame:
    df['y2_3_4'] = df['y2'] + "_" + df['y3'] +"_" + df['y4']
    df['y2_3'] = df['y2'] + "_" + df['y3']
    return df
